...
public class Beginner_mushroom_picker extends Mushroom_picker {


    public Beginner_mushroom_picker(int score, int position_x, int position_y) {
    ...
    public static void interaction_with_toxic(int x, int y, int around_x, int around_y, int index_of_beginner){
        for(int k = 0 ; k < Variables.toxicMushroomList.size() ; k++){//skanujemy po całej liście grzybow toxic i czekamy aż pętla natrafi na takowego
            if(Variables.toxicMushroomList.get(k).position_x == around_x && Variables.toxicMushroomList.get(k).position_y == around_y){
                Variables.BOARD.get(around_x).set(around_y, "X");//ustawienie na planszy że grzyb toxic jest zjedzony
                Variables.BOARD.get(x).set(y, "X");//ustawienie na planszy że beginner zginal
                Variables.toxicMushroomList.remove(k);//usunięcie grzyba toxic z arraylisty bo został zjedzony
                Variables.TOXIC_MUSH--;
                Variables.beginnersList.remove(index_of_beginner);//usuniecie begginera z listy bo zginal
                break;
            }
        }
    }

    public static void check_the_kind(int x, int y, int index_of_beginner){
        ...
        for(int i = -1 ;i<=1;i++){
            for (int j = -1; j<=1 ; j++){
                ...
                if (around_x >= 0 && around_y >= 0 && around_x < Variables.FOREST_HEIGHT && around_y < Variables.FOREST_WIDTH){
                    ...
                    else if(Variables.BOARD.get(around_x).get(around_y).equals("P")) {//P to sa toxic grzyby
                        interaction_with_toxic(x, y, around_x, around_y, index_of_beginner);
                        break outerLoop;
                    }
                    else if(Variables.BOARD.get(around_x).get(around_y).equals("@")) {//@ to sa halucynogenne grzyby//szerze to to można wywalic
                        interaction_with_hallucination(around_x, around_y);
                        //break outerLoop;pomimo że to jest to nie może tego byc bo beginner nic nie robi z tym grzybem
                    }
