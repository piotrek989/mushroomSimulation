    ...
    public static void main(String[] args) {
        ...
        while(true) {
            for (int i = 0,k = 0, m = 0; m < Variables.advancedList.size() || i < Variables.beginnersList.size() || k < Variables.intermediateList.size(); i++ ,k++, m++) {//sprawdza co jest wokół grzybiarzy od 1 do ostatniego!
                if(i<Variables.beginnersList.size()) {//musimy sprawdzac ten warunek
                ...
                if(k<Variables.intermediateList.size()) {//musimy sprawdzac ten warunek
                ...
                if(m<Variables.advancedList.size()){//musimy sprawdzac ten warunek
                    Advanced_mushroom_picker.check_the_kind(Variables.advancedList.get(m).position_x, Variables.advancedList.get(m).position_y, m);
                }
            ...
            for (int n = 0; n < Variables.beginnersList.size(); n++) {//random walk
                Beginner_mushroom_picker.random_walk(Variables.beginnersList.get(n).position_x, Variables.beginnersList.get(n).position_y, "B");//sprawdzamy czy wokól postaci begginer nie ma czasem grzyba jadalnego którego może zebrac
            }
            for (int u = 0; u < Variables.intermediateList.size(); u++) {//random walk
                Intermediate_mushroom_picker.random_walk(Variables.intermediateList.get(u).position_x, Variables.intermediateList.get(u).position_y, "I");
            }
            for (int l = 0; l < Variables.advancedList.size(); l++) {//random walk
                Advanced_mushroom_picker.random_walk(Variables.advancedList.get(l).position_x, Variables.advancedList.get(l).position_y, "A");
            }
            System.out.println();
            forest_print();
